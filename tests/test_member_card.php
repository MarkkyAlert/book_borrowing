...
check('CARD-B1',
    preg_match('/class="member-name\s+len-xl"/', $card['body']) === 1,
    'ชื่อ ' . mb_strlen(LONG_NAME) . ' ตัวอักษรได้คลาสฟอนต์เล็กสุด (len-xl)',
    '🔴 ไม่ได้ลดขนาดฟอนต์ให้ชื่อยาว');

// B2 — 🔴 ต้องคำนวณที่ฝั่งเซิร์ฟเวอร์ ไม่ใช่ JS
//     หน้านี้ต้องพิมพ์ได้แม้ JS ไม่ทำงาน
$src = (string) file_get_contents(__DIR__ . '/../admin/member_card.php');
check('CARD-B2',
    str_contains($src, 'mb_strlen($member[\'name\']'),
    'เลือกขนาดฟอนต์ที่ฝั่งเซิร์ฟเวอร์ด้วย mb_strlen — ไม่พึ่ง JS และนับตัวอักษรไทยถูก',
    '🔴 ไม่ได้คำนวณที่เซิร์ฟเวอร์ หรือใช้ strlen (จะนับเป็นไบต์ ชื่อไทยได้ขนาดผิด)');
