...
// C3b: ป้ายสถานะต้องไม่ขัดกับข้อความด้านบน
// 🧠 เคยพลาด — กล่องบอก "ไม่ให้ยืมออก" แต่ใต้กล่องขึ้นป้ายเขียว "พร้อมให้ยืม/จอง"
//    ตรวจจากข้อความที่ผู้ใช้เห็นจริง (ตัด comment/script ออกก่อน)
$visible = preg_replace('/<!--.*?-->|<script\b.*?<\/script>/s', '', $detail['body']);
check('REF-C3b', !str_contains($visible, 'พร้อมให้ยืม/จอง'),
    'ป้ายสถานะไม่ขัดกัน (ไม่ขึ้น "พร้อมให้ยืม/จอง" บนเล่มที่ยืมไม่ได้)',
    'ขึ้น "พร้อมให้ยืม/จอง" ทั้งที่บอกว่ายืมออกไม่ได้ — ข้อความขัดกันเองบนหน้าเดียว');
