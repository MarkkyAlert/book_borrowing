...
/** อ่านยอดรวมที่หัวตาราง เช่น "ทั้งหมด 4 เล่ม" */
function totalFrom(string $html): ?int
{
    return preg_match('/ทั้งหมด\s*([\d,]+)\s*(?:เล่ม|คน|รายการ)/u', $html, $m)
...
$checkPagingKeeps = function (string $id, string $url, string $mustKeep, string $label) {
    ...
    if (!$links) {
        fail($id, "🔴 ไม่เจอลิงก์เปลี่ยนหน้าเลย — ทดสอบไม่ได้ความ (ผลลัพธ์อาจพอดีหน้าเดียว "
            . 'หรือรูปแบบลิงก์เปลี่ยนไป ต้องแก้เทสต์)');
        return;
    }
    $bad = [];
    foreach ($links as $q) {
