...
class LogicalConsistencyTest
{
    ...
    private function testDR01_DoubleBorrowSameBook(): void
    {
        ...
        try {
            ...
            // Second borrow (same book - should be skipped)
            $result2 = $this->borrowService->createBorrow($this->testMemberA, [$this->testBookAlpha], 7);
            
            $borrowCount = $this->countBorrows($this->testMemberA, $this->testBookAlpha);
            
            $this->assert($testName,
                $result1['success'] === true
                && count($result2['skipped']) > 0
                && $borrowCount === 1,
                "first={$result1['success']}, second: skipped=" . count($result2['skipped']) . ", borrowCount={$borrowCount}"
            );
        } catch (Exception $e) {
            $this->fail($testName, $e->getMessage());
        }
    ...
    private function testIS02_BorrowNoStock(): void
    {
        ...
        try {
            ...
            $result = $this->borrowService->createBorrow($this->testMemberB, [$this->testBookGamma], 7);
            ...
            $this->assert($testName,
                ($result['success'] === false || count($result['skipped']) > 0)
                && $afterAvailable === 0 // ไม่ติดลบ
                && $afterAvailable >= 0
                && !$borrowExists,
                "success={$result['success']}, skipped=" . count($result['skipped']) . ", available={$afterAvailable}, borrowExists={$borrowExists}"
            );
        } catch (Exception $e) {
            $this->fail($testName, $e->getMessage());
        }
    ...
    private function testIS03_BorrowSameBookAgain(): void
    {
        ...
        try {
            ...
            // Try to borrow again
            $result = $this->borrowService->createBorrow($this->testMemberA, [$this->testBookAlpha], 7);
            ...
            $this->assert($testName,
                ...
                "skipped=" . count($result['skipped']) . ", borrowCount: {$beforeCount}→{$afterCount}"
            );
        } catch (Exception $e) {
            $this->fail($testName, $e->getMessage());
        }
