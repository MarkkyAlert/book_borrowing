...
// ============================================================
// W. ใบที่พิมพ์ออกมาต้องอยู่ในกระดาษ A4
// ============================================================
//
// 🔴 [UAT รอบ 4] ใบตามหนังสือค้างส่งกว้าง 193.6mm · ใบค้างชำระ 194.2mm
//    แต่ A4 หัก @page margin 15mm สองข้างแล้วพิมพ์ได้แค่ 180mm
//    → คอลัมน์ขวาสุดโดนตัด ซึ่งคือ "โทรแล้วเมื่อ" กับ "ค่าปรับ" พอดี
//    สาเหตุ: ชื่อไทยไม่มีช่องว่างคั่นคำ เบราว์เซอร์ตัดบรรทัดไม่ได้ เลยดันตารางกว้าง
//
// ⚠️ **ข้อจำกัดที่ต้องรู้ก่อนอ่านผลหมวดนี้**
//    PHP วัดความกว้างของ layout ไม่ได้ — ตัวเลข 193.6/194.2/180 mm ข้างบน
//    มาจากการวัดใน **เบราว์เซอร์** ด้วยมือ ไม่ใช่จากยามชุดนี้
//    ยามชุดนี้ตรวจได้แค่ "เงื่อนไขที่จำเป็น" สองข้อ ซึ่งถ้าขาดไปแล้วพังแน่ ๆ
//    ถ้าจะเปลี่ยนโครงใบพิมพ์ ต้องเปิดในเบราว์เซอร์วัดใหม่เองเสมอ
echo "\n── W. ใบพิมพ์ต้องอยู่ในกระดาษ ──\n";

$pdfSrc = (string) file_get_contents(__DIR__ . '/../admin/export_pdf.php');

// W1 — ต้องมีกติกาตัดคำในเซลล์ ไม่งั้นชื่อไทยยาว ๆ ดันตารางทะลุกระดาษ
//      🧠 ตรวจระดับซอร์สแบบเดียวกับ CN-C3 เพราะวัดความกว้างจริงใน PHP ไม่ได้
$hasWrap = (bool) preg_match('/th,\s*td\s*\{[^}]*overflow-wrap:\s*anywhere/s', $pdfSrc);
check('RPT-W1', $hasWrap,
    'เซลล์ในใบพิมพ์มีกติกาตัดคำ (overflow-wrap: anywhere) — ชื่อไทยยาวขึ้นบรรทัดใหม่แทนดันตารางกว้าง',
    '🔴 ไม่มีกติกาตัดคำ — ชื่อหนังสือไทยยาว ๆ จะดันตารางทะลุขอบกระดาษ คอลัมน์ขวาสุดหาย');

// W2 — ขอบเขตกระดาษต้องยังเป็น A4 ตามที่วัดไว้
//      ถ้ามีคนเปลี่ยน margin หรือขนาดกระดาษ ตัวเลขที่วัดมาทั้งหมดใช้ไม่ได้แล้ว
$pageOk = (bool) preg_match('/@page\s*\{[^}]*size:\s*A4[^}]*margin:\s*15mm/si', $pdfSrc);
check('RPT-W2', $pageOk,
    'ใบพิมพ์ยังเป็น A4 margin 15mm (= พิมพ์ได้ 180mm) ตรงกับที่วัดความกว้างไว้',
    '🔴 ขนาดกระดาษ/ขอบเปลี่ยนไป — ต้องเปิดในเบราว์เซอร์วัดความกว้างตารางใหม่ทุกใบ');

// W3 — เพดานจำนวนคอลัมน์ของทุกใบ
//      🧠 ที่มาของเลข 8: padding เซลล์ 8px 10px = 20px ≈ 5.3mm ต่อคอลัมน์
//         9 คอลัมน์ → padding อย่างเดียวกิน 47.7mm เหลือเนื้อหา 132mm = 14.7mm/คอลัมน์
//         ซึ่งใส่วันที่ dd/mm/yyyy (~25mm) ไม่ลง ต่อให้ตัดคำได้ก็อ่านไม่ออก
//      ⚠️ ถ้าจะเพิ่มเป็น 9 คอลัมน์ ต้องเปิดวัดในเบราว์เซอร์ก่อน แล้วมาแก้เลขนี้พร้อมเหตุผล
$MAX_COLS = 8;
$tooWide = [];
foreach (['books', 'members', 'revenue', 'overdue', 'borrows', 'unpaid', 'dormant', 'due_soon'] as $type) {
    $c = getReportConfig($type, date('Y-m-01'), date('Y-m-d'), $reportRepo);
    $cols = count($c['headers']) + 1;   // +1 = คอลัมน์ลำดับ (#) ที่ใบพิมพ์เติมให้เอง
    if ($cols > $MAX_COLS) $tooWide[] = "{$type}={$cols}";
}
check('RPT-W3', $tooWide === [],
    "ทุกใบมีไม่เกิน {$MAX_COLS} คอลัมน์ — อยู่ในงบความกว้างของ A4",
    '🔴 คอลัมน์เกินงบกระดาษ: ' . implode(' · ', $tooWide) . " (เพดาน {$MAX_COLS})");
