 ...
 */
/**
 * 🔴 ป้ายแดงนับ 2 อย่างรวมกัน: งานประจำวัน + สุขภาพระบบ
 *
 * 🧠 เจอตอนรันบน clone ที่ติดตั้งสด: ที่นั่นงานประจำวันเป็น 0 ทั้งหมด
 *    แต่ install.php ยังไม่ถูกลบ → กระดิ่งขึ้น 1 ซึ่ง **ถูกต้องแล้ว**
 *    เทสต์เดิมเทียบกับ getAlertCounts() อย่างเดียวจึงฟ้องผิด
 *    บนเครื่องพัฒนามองไม่เห็นเพราะมีของค้างหลักสิบกลบไว้
 */
$health     = (new \App\Services\DashboardService($pdo))->getSystemHealth();
$expectBell = $bell['total'] + $health['admin_total'];
$hasBadge   = (bool) preg_match('/aria-label="การแจ้งเตือน".{0,500}?bg-red-500/s', $adminHome);
check('BELL-B1', $hasBadge === ($expectBell > 0),
    $expectBell > 0
        ? "งานประจำวัน {$bell['total']} + สุขภาพระบบ {$health['admin_total']} → ป้ายแดงขึ้น"
        : 'ไม่มีอะไรค้างเลยทั้งสองกลุ่ม → ป้ายแดงไม่ขึ้น',
    $expectBell > 0
...
// 🧠 ข้อความ "ไม่มีอะไรค้าง" ต้องขึ้นเฉพาะตอนว่างจริงทั้งสองกลุ่ม
//    ถ้ามีเรื่องสุขภาพระบบอยู่ การขึ้นว่า "ไม่มีอะไรค้าง" จะเป็นการโกหก
check('BELL-B3', str_contains($adminHome, 'ไม่มีอะไรค้าง') === ($expectBell === 0),
    $expectBell === 0
        ? 'ว่างจริง → ขึ้นข้อความ "ไม่มีอะไรค้าง"'
        : 'มีของค้าง → ไม่ขึ้นข้อความว่าง',
    $expectBell === 0
        ? '🔴 ไม่มีข้อความสำหรับสถานะว่าง — เมนูจะโล่งโดยไม่บอกอะไร'
        : '🔴 มีเรื่องค้างอยู่แต่ยังบอกว่า "ไม่มีอะไรค้าง"');
