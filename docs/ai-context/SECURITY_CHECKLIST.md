...
| Rate limit ของ API ค้นหา | ✅ 🧪 | **300 ครั้ง / 5 นาที ต่อ IP** — ทดสอบแล้วบล็อกที่ครั้งที่ 301 พอดี · โควตาสูงเพราะห้องสมุดที่ใช้ NAT แชร์ IP เดียวกันทั้งหมด (ดู F-34) |
| 🔴 เคยพัง: ค้นหาเรียก `checkRateLimit()` แต่ลืม `incrementRateLimit()` | ✅ แก้แล้ว | ตัวนับเป็น 0 ตลอดกาล → ยิง 200 ครั้งไม่ถูกบล็อกเลย · มี **EC-11b** คุมไว้แล้ว (ดู F-34) |
...
| ↳ ข้อความที่คนพลาดได้รับ | "ไม่มีเล่มว่าง" ✅ (เดิม ~25% เจอ SQLSTATE ดิบ — แก้แล้วด้วย retry ดู **F-20**) |
