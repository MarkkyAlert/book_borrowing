...
> F-15…F-18 เจอจากการทดสอบทุก flow ผ่าน Chrome จริง — **F-15, F-16 แก้แล้ว** · F-17, F-18 เป็น UX ยังไม่แก้
...
## 🔵 F-17 — [ยังไม่แก้] ปุ่มลบเปิดใช้งานได้ ทั้งที่ลบไม่มีวันสำเร็จ
...
**แนวทางแก้:** ให้ query ที่ดึงรายการส่งสถานะ "ลบได้/ลบไม่ได้ + เหตุผล" มาด้วย แล้ว disable ปุ่มพร้อม tooltip ให้ตรงเหตุผล

---

## 🔵 F-18 — [ยังไม่แก้] รูปแบบวันที่ในรายงานไม่ตรงกันระหว่างหน้าจอกับ PDF

รายงาน "หนังสือค้างส่ง" แสดงวันที่เป็น `2026-06-21` บนหน้าเว็บ แต่เป็น `21/06/2026` ในหน้า PDF
สาเหตุ: `getReportConfig()` ส่ง `$forPdf` เข้าไปคุม `getOverdueReport(bool $formatDate)` — จัดรูปแบบวันที่ให้เฉพาะตอน PDF
ทั้งระบบใช้ `d/m/Y` เป็นมาตรฐาน (ดู `formatDate()`) หน้าจอจึงควรแสดงแบบเดียวกัน
