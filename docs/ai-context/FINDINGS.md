...
→ ชุดเต็มจาก **150 → 315 เคส** ใน 19 ชุด (7.7 วินาที)
...
**ทดสอบหลังแก้:** ชุดเต็ม **315/315** · รันซ้ำ 2 รอบได้จำนวนแถวใน 4 ตารางเท่าเดิมเป๊ะ
(ไม่สะสมขยะ) · `rebuild_search_index --check` ผ่าน · stock invariant ถูกต้องทุกเล่ม

**ยังเหลือ 19 ไฟล์ที่ไม่ได้ต่อเข้าชุด** — ยังไม่ตัดสินใจ:
...
- ที่เหลือส่วนใหญ่ไม่คืน exit code → ตัวรันชุดจับ fail ไม่ได้ ต้องแก้ก่อนถ้าจะเก็บไว้
