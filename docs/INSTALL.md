...
**🔴 ขั้นที่ 5.1 — สร้าง index ค้นหา (บังคับถ้าทำขั้นที่ 5)**

```bash
php database/rebuild_search_index.php
```

การค้นหาหนังสือใช้ FULLTEXT บนคอลัมน์ที่ **PHP เป็นคนสร้าง** (SQL คำนวณเองไม่ได้)
หนังสือที่มาจากไฟล์ `.sql` จึงยังไม่มี index — **จะแสดงในรายการปกติ แต่ค้นหาไม่เจอ**

ตรวจว่าครบแล้วหรือยังด้วย:

```bash
php database/rebuild_search_index.php --check
```

> 📌 ถ้าติดตั้งผ่าน `install.php` อย่างเดียว (ไม่ import ไฟล์ตัวอย่าง) ไม่ต้องทำขั้นนี้
> ตัวติดตั้งสร้าง index ให้เรียบร้อยแล้ว
