...
> 🔴 **ถ้า deploy ด้วย `git clone` — โฟลเดอร์ `.git` จะอยู่ใน web root ด้วย**
> `.htaccess` ที่รากบล็อกไว้ให้แล้ว (ตอบ 404) แต่ **ถ้า server ไม่อ่าน `.htaccess`**
> (Nginx, หรือ Apache ที่ตั้ง `AllowOverride None`) การป้องกันจะไม่ทำงาน
> → ใครก็ดาวน์โหลด `.git/objects/pack/*.pack` แล้ว **กู้ซอร์สโค้ดทั้งโปรเจกต์**
> พร้อมประวัติ commit ได้ทันที
>
> **ทางที่ปลอดภัยกว่าสำหรับ production:** อย่าอัปโหลดโฟลเดอร์ `.git` ขึ้น server เลย
> ```bash
> git archive --format=tar HEAD | tar -x -C /path/to/webroot
> ```
> หรือ clone ไว้นอก web root แล้วคัดลอกเฉพาะไฟล์ที่ต้องใช้
>
> **ตรวจว่าปลอดภัยแล้วหรือยัง:** เปิด `https://โดเมนของคุณ/.git/config` ในเบราว์เซอร์
> — ต้องได้ 404 ถ้าเห็นข้อความในไฟล์แสดงว่ายังเปิดอยู่
...
ระบบมี 2 งานอัตโนมัติที่ **ตั้งก็ได้ ไม่ตั้งก็ได้** — ทั้งคู่มีทางสำรองในตัวระบบอยู่แล้ว

> 🧠 **ระบบนี้ออกแบบมาให้ไม่ต้องพึ่ง cron** เพราะลูกค้ากลุ่มเป้าหมาย
> (ห้องสมุดที่ดูแลคนเดียว ไม่มีฝ่าย IT) มักตั้ง cron เองไม่เป็น
> ถ้าออกแบบให้ *ต้อง* ตั้ง แล้วเขาตั้งไม่เป็น ระบบจะพังเงียบโดยไม่มีใครรู้
> cron จึงเป็น "ของแถมให้ดีขึ้น" ไม่ใช่ "ของที่ขาดไม่ได้"
...
| **ความถี่** | ทุก 15 นาที |
| **ถ้าไม่ตั้ง** | ✅ **ระบบเคลียร์ให้เองอยู่แล้ว** — ทุกครั้งที่มีคนเปิดหน้าที่เกี่ยวกับการจอง<br>(หน้าภาพรวม / รายการจอง / รายการหนังสือ / **หน้าแรกฝั่งสมาชิก**) ระบบจะปล่อยการจองที่หมดอายุ<br>และคืนหนังสือขึ้นชั้นทันที<br>⚠️ **ข้อเสียคือ "รอจนกว่าจะมีคนเปิด"** — ถ้าห้องสมุดปิดยาว 5 วันโดยไม่มีใคร<br>เปิดคอมเลย หนังสือที่คนจองไม่มารับจะถูกกันไว้ตลอด 5 วันนั้น<br>ตั้ง cron ช่วยให้หนังสือกลับขึ้นชั้นเร็วขึ้น แต่**ไม่ตั้งก็ไม่มีอะไรหาย** |

**📋 ยืนยันด้วยการทดสอบจริง (UAT รอบ 3):** สร้างการจองหมดอายุค้างไว้ 5 รายการ
(หมดอายุมาแล้ว 1–5 วัน หนังสือยังถูกกันอยู่ทั้ง 5 เล่ม) → **เปิดหน้า "รายการจอง"
เพียงครั้งเดียว** ทั้ง 5 รายการเปลี่ยนเป็น `expired` และหนังสือกลับขึ้นชั้นครบ
สต็อกตรงสูตรทุกเล่ม
...
| **ถ้าไม่ตั้ง** | ✅ **ระบบล้างให้เองอยู่แล้ว** — ทุกครั้งที่มีคนกดขอลิงก์รีเซ็ตรหัสผ่าน<br>ระบบจะลบ token ที่หมดอายุทิ้งก่อน (`AuthService::requestPasswordReset()`)<br>⚠️ ถ้าไม่มีใครขอรีเซ็ตเลยนาน ๆ token เก่าจะค้างอยู่ — ไม่กระทบการใช้งาน<br>เป็นเรื่องความสะอาดของตารางเท่านั้น |
...
|-------------------|----------|---------|
| **Database** | สูงมาก | บรรณารักษ์ทำเองได้ที่ ตั้งค่าระบบ → สำรองข้อมูล · ผู้ดูแลเซิร์ฟเวอร์ใช้ phpMyAdmin / `mysqldump` ก็ได้ |
...
### ✅ ยืนยันแล้วว่า backup/restore ใช้ได้จริง

ทดสอบเมื่อ 2026-08-28 หลังเปลี่ยนโครงสร้างฐานข้อมูลหลายรอบ — `mysqldump` เก็บมาครบทุกอย่าง:
FULLTEXT index, CHECK constraint, FOREIGN KEY, UNIQUE, ข้อมูลภาษาไทย
· กู้คืนลงเครื่องใหม่แล้ว **การค้นหายังทำงาน** และรันชุดทดสอบเต็มผ่าน 395/395

> 🔴 **หลังกู้คืนลงเครื่องใหม่ ให้รัน 2 คำสั่งนี้เสมอ**
> ```bash
> php database/migrate.php                      # เผื่อ backup มาจากเวอร์ชันเก่ากว่า
> php database/rebuild_search_index.php --check # ตรวจว่า index ค้นหาครบ
> ```
> และ **ตรวจว่า `install.php` ถูกล็อกแล้ว** — เปิด `https://โดเมน/install.php`
> ต้องขึ้นว่า "ระบบได้รับการติดตั้งเรียบร้อยแล้ว" ถ้าขึ้นหน้าฟอร์มติดตั้ง
> แปลว่ายังไม่ถูกล็อก ใครก็สร้างบัญชีผู้ดูแลระบบของตัวเองได้ (ดู F-23, F-31)
...
| ระบบ email แจ้งเตือน | ไม่มีอีเมลแจ้งเตือนใกล้ครบกำหนด/เกินกำหนด (ตั้งใจไม่ทำ) · **ส่งได้เฉพาะลิงก์ตั้งรหัสผ่านใหม่** และปิดไว้เป็นค่าเริ่มต้น |

### คำเตือนสำหรับการใช้งานจริง

- **Password Reset:** ตั้ง SMTP ที่ **ตั้งค่าระบบ → อีเมล (สำหรับลิงก์รีเซ็ตรหัสผ่าน)** แล้วกด "ทดสอบส่ง" ให้ผ่านก่อน จากนั้นหน้า "ลืมรหัสผ่าน" จะส่งลิงก์ตั้งรหัสใหม่ให้ทางอีเมลได้ · 🔴 **ค่าเริ่มต้นคือปิด** ถ้าไม่ตั้ง SMTP ผู้ดูแลตั้งรหัสให้ที่หน้าจัดการสมาชิกได้ (ระบบบังคับให้เจ้าตัวเปลี่ยนรหัสทันทีที่เข้าใช้) — **ไม่ต้องไปแก้ที่ฐานข้อมูล** ยกเว้นกรณีลืมรหัส admin ตัวเดียวในระบบ ดู README หัวข้อ "ลืมรหัสผ่าน Admin"
