...
```
ระดับ 0 (ง่ายที่สุด): ตั้งค่าในหน้าเว็บ       → กฎการยืม-คืน วันหยุดห้องสมุด บัตรสมาชิก
ระดับ 1 (ง่ายมาก):  แก้ค่าใน .env          → ชื่อเว็บ URL รหัส DB (ไม่ต้องแตะโค้ด)
...
| **ค่าปรับต่อวัน** | **หน้าเว็บ** | Admin → ตั้งค่าระบบ → กฎการยืม-คืน |
| **จำนวนวันยืม** | **หน้าเว็บ** | Admin → ตั้งค่าระบบ → กฎการยืม-คืน |
| **จำนวนเล่มสูงสุดที่ยืมได้** | **หน้าเว็บ** | Admin → ตั้งค่าระบบ → กฎการยืม-คืน |
| **วันที่ห้องสมุดปิด (ไม่คิดค่าปรับ)** | **หน้าเว็บ** | Admin → ตั้งค่าระบบ |
...
| อยากแก้ | แก้ที่ไฟล์ |
|--------|----------|
| จำนวนเล่มสูงสุดที่ยืมได้ | **Admin → ตั้งค่าระบบ → กฎการยืม-คืน** |
| จำนวนวันยืม default | **Admin → ตั้งค่าระบบ → กฎการยืม-คืน** |
...
| ค่าปรับต่อวัน | **Admin → ตั้งค่าระบบ → กฎการยืม-คืน** |
...
→ เข้าระบบเป็นผู้ดูแล → **ตั้งค่าระบบ** → ฟอร์ม **"กฎการยืม-คืน"** → ใส่ตัวเลข → บันทึก → มีผลทันที

> ⚠️ **ไม่ต้องแก้ไฟล์ `.env`** — ระบบอ่านหน้าตั้งค่าก่อน `.env` เสมอ
> พอกดบันทึกในหน้าเว็บครั้งแรก ค่าใน `.env` จะไม่ถูกใช้อีกเลย

---

### "อยากเปลี่ยนจำนวนเล่มที่ยืมได้"

→ **ตั้งค่าระบบ** → **"กฎการยืม-คืน"** → ตั้งแยกได้ระหว่างสมาชิกทั่วไปกับเจ้าหน้าที่ → บันทึก
...
| ระดับ | แก้อะไร | ไฟล์ |
|------|--------|------|
| 🟢 **ง่ายที่สุด** | ค่าปรับ, จำนวนวันยืม, โควตา, วันหยุดห้องสมุด | **หน้าตั้งค่าระบบ** (ไม่ใช่ไฟล์) |
| 🟢 **ง่ายมาก** | ชื่อเว็บ, URL, รหัส DB | `.env` |
