...
class BorrowService
{
    ...
    public function createBorrow(int $userId, array $bookIds, ?int $borrowDays = null): array
    {
        ...
        // 🔴 [UAT รอบ 4] สมาชิกที่ตั้งเป็น "เลิกใช้งาน" ต้องยืมใหม่ไม่ได้
        //
        //    เดิมระบบแค่**ซ่อน**เขาออกจากดรอปดาวน์ (UserRepository::getBorrowableMembers)
        //    ซึ่งเป็นความสวยงามของหน้าจอ ไม่ใช่กฎ — แท็บที่เปิดค้างไว้ กดย้อนกลับ
        //    หรือ **สแกนบัตรสมาชิก** ยังยืมทะลุได้ทั้งหมด (บัตรเก่ายังอยู่ในมือเขา)
        //
        // 🧠 ปิดเฉพาะ "กิจกรรมใหม่" เท่านั้น — การรับคืนต้องทำได้ต่อไป
        //    ไม่งั้นหนังสือจะถูกขังอยู่กับคนที่ติดต่อไม่ได้ ซึ่งตรงข้ามกับที่ต้องการ
        if (isset($user['is_active']) && (int) $user['is_active'] !== 1) {
            throw new Exception(
                'สมาชิก "' . $user['name'] . '" ถูกตั้งเป็นเลิกใช้งาน จึงยืมหนังสือใหม่ไม่ได้'
                . ' — ถ้าต้องการให้ยืมได้อีก เปิดใช้งานคืนได้ที่หน้าแก้ไขสมาชิก'
            );
        }
    ...
    public function renewBorrow(int $borrowId, ?int $days = null): array
    {
        ...
        return runWithDeadlockRetry($this->pdo, function () use ($borrowId, $days) {
            ...
            try {
                ...
                // 🔴 [UAT รอบ 4] Step 1b: สมาชิกที่เลิกใช้งานแล้ว ต่ออายุไม่ได้
                //    การต่ออายุคือการยืดเวลาให้คนที่เลิกใช้บริการแล้ว ทำให้หนังสือกลับชั้นช้าลง
                //    เป้าหมายตอนนี้คือ "เอาหนังสือคืน" ไม่ใช่ "ให้ถือต่อ"
                // 🧠 อ่านสดจากตาราง users ไม่ใช้ค่าที่ join มา เพราะอยู่ใน transaction
                //    และ findByIdForUpdate() คืนเฉพาะคอลัมน์ของ borrows
                $renewUser = $this->userRepo->findMemberById((int) $borrow['user_id']);
                if ($renewUser && isset($renewUser['is_active']) && (int) $renewUser['is_active'] !== 1) {
                    throw new Exception(
                        'สมาชิก "' . $renewUser['name'] . '" ถูกตั้งเป็นเลิกใช้งาน จึงต่ออายุให้ไม่ได้'
                        . ' — รับคืนหนังสือได้ตามปกติ'
                    );
                }
