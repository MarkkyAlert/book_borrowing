...
class BookRepository
{
     ...
     * ==========================================================================
     * 🧠 รวม 4 คอลัมน์ที่การค้นหาครอบคลุม (title, author, isbn, call_number)
     *    ให้ตรงกับเงื่อนไข LIKE ใน buildListQuery() — ถ้าไม่ตรงกันจะเกิดอาการ
     *    "FULLTEXT บอกว่ามี แต่ LIKE กรองทิ้ง" หรือแย่กว่านั้นคือค้นบางคอลัมน์ไม่เจอเลย
     *
     * 🔴 เคยขาด call_number ไปจริง ๆ แล้วทำให้ค้นเลขเรียกแบบ LC (`PZ7.R79`) ไม่เจอเลย:
     *    buildListQuery() ใส่ทั้ง MATCH(search_tokens) และ call_number LIKE แล้วรวมด้วย AND
     *    → trigram ของ "PZ7.R79" ไปหาใน tokens ที่ไม่มีเลขเรียก → 0 แถว → AND ตัดทิ้งหมด
     *    เลขเรียกที่เป็นตัวเลขล้วน (`823.914`) รอดมาได้เพราะ buildSearchBooleanQuery()
     *    คืน null เมื่อสัดส่วนตัวเลข ≥ 70% → ตกไปใช้ LIKE ล้วน
     *    เทสต์เดิมทดสอบด้วย `999.99` ซึ่งเป็นทางที่รอดพอดี จึงเขียวมาตลอด
     *
     * ⚠️ ถ้าเพิ่มคอลัมน์ที่ค้นหาได้ ต้องแก้ 4 ที่ให้ตรงกัน:
     *    1. เงื่อนไข LIKE ใน buildListQuery()
     *    2. ฟังก์ชันนี้
     *    3. `database/rebuild_search_index.php` แล้วรัน --all ใหม่
     *    4. migration ใหม่ที่ rebuild ให้ลูกค้าที่ติดตั้งไปแล้ว
     *       (ไม่งั้นลูกค้าเก่าจะยังค้นไม่เจอ เพราะ tokens เป็นค่าที่คำนวณไว้ตอนบันทึก
     *        ไม่ได้คำนวณใหม่ตอนค้นหา)
     */
    private function makeSearchTokens(array $data): string
    {
        return buildSearchTokens(trim(
            ($data['title'] ?? '') . ' ' . ($data['author'] ?? '') . ' '
            . ($data['isbn'] ?? '') . ' ' . ($data['call_number'] ?? '')
        ));
    }
    ...
    private function buildListQuery(array $filters): array
    {
        ...
        // 🏷️ [F-48] Filter: เล่มที่ยังไม่ได้ลง ISBN — งานลงรายการของบรรณารักษ์
        // 🧠 แยกเป็นพารามิเตอร์ของตัวเอง ไม่ยัดรวมใน status
        //    เพราะ "ไม่มี ISBN" กับ "สต็อกว่าง" เป็นคนละแกน ยัดรวมกันจะเลือกได้อย่างเดียว
        //    ทั้งที่ควรหา "เล่มที่ว่างและยังไม่ได้ลง ISBN" ได้
        // ⚠️ ต้องเช็คทั้ง NULL และสตริงว่าง — ฟอร์มเพิ่มหนังสือบันทึกช่องว่างเป็น ''
