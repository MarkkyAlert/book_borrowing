...
$errors = [];
$success = false;
// 📧 ต้องโหลดตั้งแต่ต้น — ส่วนแสดงผลเรียก mailEnabled() ด้วย
//    🔴 เคยโหลดไว้ในบล็อก POST อย่างเดียว แล้วหน้าจอพังตอนเปิดแบบ GET
require_once __DIR__ . '/includes/mailer.php';

$resetLink = null; // แสดงเฉพาะ demo mode (localhost + APP_DEBUG)
$mailSent   = false;   // ส่งอีเมลสำเร็จจริงหรือไม่
$mailFailed = false;   // ส่งไม่ผ่าน — เข้า error_log ให้ผู้ดูแล ไม่แสดงบนหน้าจอ (กันการไล่เดาสมาชิก)
$email = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if (!$result['success']) {
            ...
            /**
             * 📧 ส่งลิงก์ทางอีเมล — เฉพาะเมื่อผู้ดูแลตั้งค่า SMTP ไว้แล้ว
             *
             * 🔴 **ห้ามบอกว่า "ส่งแล้ว" ถ้าส่งไม่สำเร็จ** — ต้องบอกตามจริง
             *    แล้วให้ตกกลับไปใช้ทางเคาน์เตอร์ ไม่ใช่ปล่อยให้สมาชิกนั่งรอเมลที่ไม่มา
             *
             * 🛡️ ข้อความตอบกลับต้อง **เหมือนกันทุกกรณี** ไม่ว่าอีเมลนั้นจะมีในระบบหรือไม่
             *    ไม่งั้นจะกลายเป็นเครื่องมือไล่เดาว่าใครเป็นสมาชิก (F-40)
             *    → `$mailSent` ถูกตั้งเฉพาะตอนมี token จริง แต่**ข้อความหลักไม่เปลี่ยน**
             */
            if ($result['token'] && mailEnabled()) {
                $link = APP_URL . '/reset_password.php?token=' . $result['token'];
                $sent = sendMail(
                    $email,
                    'ตั้งรหัสผ่านใหม่ — ' . APP_NAME,
                    "มีการขอตั้งรหัสผ่านใหม่สำหรับบัญชีนี้

"
                    . "เปิดลิงก์ด้านล่างเพื่อตั้งรหัสผ่านใหม่ (ใช้ได้ครั้งเดียว หมดอายุใน 1 ชั่วโมง)
"
                    . $link . "
...
"
                ...
                if ($sent['success']) {
                    ...
                    // 📝 บันทึกสาเหตุไว้ให้ผู้ดูแลตามได้ แต่ไม่โชว์รายละเอียดให้ผู้ใช้ทั่วไป
                    error_log('[forgot_password] ส่งเมลไม่สำเร็จ: ' . $sent['error']);
                    $mailFailed = true;
                }
            }

            // 🧠 Demo mode: แสดง reset link เฉพาะ localhost + APP_DEBUG=true
            //    ใช้ตอนพัฒนา/ตรวจงาน เมื่อยังไม่ได้ตั้งค่า SMTP
        ...
        <div class="bg-white rounded-3xl shadow-2xl p-8 sm:p-10 border border-gray-200">
            <div class="text-center mb-8">
                ...
                <p class="mt-2 text-sm text-gray-600">
                    <?php // 🔴 ข้อความต้องตรงกับสิ่งที่ระบบทำจริง ณ การตั้งค่าปัจจุบัน
                          //    ปิดอีเมลอยู่แล้วเขียนว่า "จะส่งลิงก์ให้" = สัญญาที่ทำไม่ได้ ?>
                    <?php if (mailEnabled()): ?>
            ...
            <?php if ($success): ?>
                ...
                <div class="bg-green-50 border-l-4 border-green-500 p-4 mb-6 rounded-r-lg">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            ...
                            <?php // 🛡️ [F-40] ข้อความหลักต้องเหมือนกันทุกกรณี ไม่บอกว่าอีเมลนี้มีในระบบไหม
                                  //    ไม่งั้นหน้านี้จะกลายเป็นเครื่องมือไล่เดาว่าใครเป็นสมาชิก ?>
                            <p class="text-sm text-green-700 mt-1">
                                <?php // 🔴 เขียนว่า "จะส่ง" ไม่ใช่ "ส่งแล้ว" โดยตั้งใจ
                                      //    ถ้าเขียนว่า "ส่งแล้ว" แล้ว SMTP ล้ม = โกหกผู้ใช้
                                      //    และถ้าไปผูกข้อความกับผลการส่งจริง จะกลายเป็นการบอก
                                      //    ว่าอีเมลนี้มีในระบบหรือไม่ (F-40) — เลือกถ้อยคำที่จริงทั้งสองทาง ?>
                                <?php if (mailEnabled()): ?>
                                    หากอีเมลนี้มีอยู่ในระบบ ระบบจะส่งลิงก์ตั้งรหัสผ่านใหม่ไปให้
                                <?php else: ?>
                ...
                <?php
                /**
                 * 🔴 **ห้ามแยกข้อความตามผลการส่ง** — ดูเหมือนจะช่วยผู้ใช้ แต่ทำให้หน้านี้
                 *    กลายเป็นเครื่องมือไล่เดาว่าอีเมลไหนเป็นสมาชิก (F-40):
                 *    ส่งไม่ผ่าน = มีบัญชีจริงแน่ ๆ · ไม่ขึ้นอะไร = ไม่มีบัญชี
                 *
                 * 🧠 ทางออก: ข้อความเดียวกันทุกกรณี + **บอกทางสำรองไว้เสมอ**
                 *    ผู้ใช้ที่ไม่ได้รับเมล (ไม่ว่าเพราะอะไร) จะรู้เองว่าต้องไปที่เคาน์เตอร์
                 *    ส่วนสาเหตุที่ส่งไม่ผ่านเข้า error_log ให้ผู้ดูแลตามได้
                 *    และผู้ดูแลมี "ปุ่มทดสอบส่งเมล" ในหน้าตั้งค่าไว้ตรวจโดยตรงอยู่แล้ว
                 */
                ?>
                <?php if (mailEnabled()): ?>
                    <div class="bg-gray-50 border border-gray-200 p-4 mb-6 rounded-xl">
                        <p class="text-sm font-medium text-gray-800 mb-1.5">
                            <i class="bi bi-envelope-check mr-1.5 text-gray-500"></i>ขั้นตอนถัดไป
                        </p>
                        <p class="text-sm text-gray-600">
                            เปิดกล่องจดหมายของคุณแล้วกดลิงก์ในอีเมล (ตรวจถังสแปมด้วย)
                            ลิงก์ใช้ได้ครั้งเดียวและหมดอายุใน 1 ชั่วโมง<br>
                            <strong>ถ้าไม่ได้รับภายในไม่กี่นาที</strong>
                            ติดต่อเคาน์เตอร์ห้องสมุดพร้อมแสดงบัตรสมาชิก เจ้าหน้าที่ตั้งรหัสให้ได้ทันที
                        </p>
                    </div>
                <?php else: ?>
                    <div class="bg-gray-50 border border-gray-200 p-4 mb-6 rounded-xl">
                ...
                <form class="space-y-6" method="POST" novalidate>
                    ...
                    <div>
                        <button type="submit" class="group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-bold rounded-xl text-white bg-amber-500 hover:bg-amber-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500 shadow-lg shadow-amber-500/30 transition-all hover:-translate-y-0.5">
                            ...
                            <?= mailEnabled() ? 'ส่งลิงก์ตั้งรหัสผ่านใหม่' : 'แจ้งคำขอตั้งรหัสผ่านใหม่' ?>
                        </button>
                    </div>
                </form>
