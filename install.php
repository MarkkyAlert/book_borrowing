...
//
// 🔴 [ทดสอบแล้ว 2026-08-30 ผ่าน Apache จริง] สองชั้นนี้เป็น **OR** ไม่ใช่ AND
//    ชั้นใดชั้นหนึ่งติด = ติดตั้งซ้ำไม่ได้ ดังนั้นการจะติดตั้งใหม่ต้องปลดทั้งสองชั้น
//    เดิมหน้าจอเขียนว่า "ลบ .installed หรือเพิ่ม ?force=1" ซึ่งผิดทั้งคู่:
//      - ลบ .installed อย่างเดียวไม่พอ (ล็อคใน DB ยังอยู่)
//      - `force` ไม่มีอยู่ในโค้ดนี้เลยสักบรรทัด เป็นข้อความค้างจากก่อนมีล็อคชั้น DB
//    ⚠️ ห้ามใส่ทางลัดแบบ ?force=1 กลับเข้ามา — หน้านี้เข้าถึงได้โดยไม่ต้อง login
//       เท่ากับเปิดให้ใครก็ได้ล้างระบบแล้วสร้างบัญชี admin ของตัวเอง
                    ...
                    <div class="card shadow-sm border-warning">
                        ...
                        <div class="card-footer bg-light">
                            <small class="text-muted">
                                ...
                                <strong>ติดตั้งใหม่:</strong> ต้องปลดล็อค <strong>ทั้ง 2 ชั้น</strong> —
                                ลบไฟล์ <code>.installed</code> <em>และ</em> ลบแถว
                                <code>installed_at</code> ในตาราง <code>settings</code>
                                (ทำอย่างใดอย่างหนึ่งไม่พอ)
                            </small>
                        </div>
                    </div>
                </div>
    ...
    try {
        ...
        $pdo->exec("
            CREATE TABLE IF NOT EXISTS `users` (
                ...
                `is_active` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '1 = ยังใช้งาน · 0 = เลิกใช้งาน (ซ่อนจากดรอปดาวน์ + ล็อกอินไม่ได้ แต่ประวัติยังอยู่)',
