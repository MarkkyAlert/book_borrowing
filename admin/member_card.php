    ...
    <div class="card">
        <div class="main-content">
            ...
            <div class="member-details">
                <?php
                // 📏 เลือกขนาดฟอนต์ตามความกว้างที่ชื่อ **ต้องใช้จริง**
                //
                // 🔴 [UAT รอบ 2] เดิมใช้ mb_strlen ซึ่งนับสระบน-ล่างและวรรณยุกต์เป็นตัวอักษรด้วย
                //    ทั้งที่อักขระพวกนี้ซ้อนบนพยัญชนะ ไม่กินความกว้าง → ชื่อไทยถูกประเมินยาวเกินจริง
                //    ผลคือสมาชิก 30 จาก 206 คนโดนย่อเหลือ 14px ทั้งที่ใส่ 16px ได้สบาย
                //    และชื่อที่ยาวที่สุดโดนย่อเหลือ 10.5px ทั้งที่เหลือที่ว่างใต้ชื่อ 27px
                //    (ดู displayNameLength() ใน includes/functions.php)
                //
                // 🧠 เกณฑ์ข้างล่างมาจากการ**วัดจริง** ไม่ได้เดา — เรนเดอร์บัตรแล้ววัดด้วย DOM
                //    กล่องชื่อกว้าง 180px × เพดาน 3 บรรทัด = งบความกว้าง 540px
                //    จำนวนอักขระที่ยัดได้จริงภายใน 3 บรรทัด (เลือกค่าที่แคบกว่าระหว่างไทย/ละติน):
                //      16px → 45   ·   14px → 59   ·   12px → 76   ·   10.5px → 82
                //    เกณฑ์จริงตั้งต่ำกว่านั้นราว 10% เผื่อฟอนต์/เครื่องพิมพ์ต่างกัน
                //
                // ⚠️ ถ้าจะขยับตัวเลข ต้องวัดใหม่ก่อน — ยาม CARD-B4 กันไม่ให้ตั้งเกินความจุที่วัดไว้
                $nameLen = displayNameLength($member['name']);
                $nameClass = match (true) {
                    $nameLen > 68 => 'len-xl',   // 10.5px — ชื่อยาวผิดปกติจริง ๆ
                    $nameLen > 53 => 'len-lg',   // 12px
                    $nameLen > 40 => 'len-md',   // 14px
                    default       => '',         // 16px เต็ม — ชื่อไทยเกือบทั้งหมดอยู่ตรงนี้
                };
                ?>
