...
function ruleDefinitions(): array
{
    return [
        ...
        'MAX_BORROW_BOOKS' => [
            ...
            'min'     => 1,
            'max'     => 100,
            'label'   => 'ยืมได้สูงสุด — สมาชิกทั่วไป',
            'unit'    => 'เล่ม',
            'help'    => 'นับรวมทั้งที่ยืมอยู่และที่จองไว้',
        ],
        // 👔 [โควตาตาม role] ห้องสมุดทั่วไปมักให้เจ้าหน้าที่ยืมได้มากกว่าสมาชิกทั่วไป
        //    🧠 ตั้งเป็นกฎแยก ไม่ใช่ตารางใหม่ — ระบบมีแค่ 3 role และ admin/staff ใช้เพดานเดียวกัน
        //       ทำเป็นตาราง role_quotas จะได้ตารางที่มี 2 แถวตลอดกาล แลกกับ schema ที่ต้องดูแลเพิ่ม
        //    ⚠️ ค่า default เท่ากับของสมาชิกทั่วไป → ลูกค้าเดิมที่อัปเกรดพฤติกรรมไม่เปลี่ยน
        //       จนกว่าจะตั้งค่าเอง
        'MAX_BORROW_BOOKS_STAFF' => [
            'setting' => 'rule_max_books_staff',
            'env'     => 'MAX_BORROW_BOOKS_STAFF',
            'default' => 3,
            'min'     => 1,
            'max'     => 100,
            'label'   => 'ยืมได้สูงสุด — เจ้าหน้าที่',
            'unit'    => 'เล่ม',
            'help'    => 'ใช้กับ role เจ้าหน้าที่และผู้ดูแลระบบ · ตั้งเท่ากับสมาชิกทั่วไปได้ถ้าไม่ต้องการแยก',
        ],
        'FINE_PER_DAY' => [
...
/**
 * ==========================================================================
 * 🎯 จุดประสงค์: เพดานการยืมของ role นั้น ๆ — **แหล่งเดียวของทั้งระบบ**
 * ==========================================================================
 *
 * 📥 Input: @param string|null $role ('member' | 'staff' | 'admin' | null)
 * 📤 Output: @return int จำนวนเล่มสูงสุดที่ยืมได้ (รวมที่จองไว้)
 *
 * 🔴 [สำคัญ] ทุกที่ที่เคยใช้ MAX_BORROW_BOOKS ตรง ๆ ต้องเปลี่ยนมาเรียกตัวนี้
 *    ระบบมี **9 จุด** ที่ตัดสินเรื่องโควตา ข้าม 5 ไฟล์:
 *      ด่านยืม · ด่านจอง · ด่านต่อคิว · ด่านเลื่อนคิว ·
 *      ตัวกรอง "เต็มโควตา" · หน้ารายชื่อสมาชิก · ฟอร์มบันทึกการยืม
 *    ถ้าเหลือจุดใดใช้ค่าเดิม จะเกิดสภาพ **หน้าจอบอกอย่าง ระบบทำอีกอย่าง**
 *    เช่น หน้าจอให้เลือกได้ 3 เล่ม แต่ด่านหลังบ้านยอม 10
 *
 * 🧠 role ที่ไม่รู้จัก / null → ใช้เพดานของสมาชิกทั่วไป (ค่าที่เข้มที่สุด)
 *    ปลอดภัยกว่าเดาว่าเป็นเจ้าหน้าที่แล้วให้ยืมเกิน
 */
function quotaForRole(?string $role): int
{
